...
|-----------|------|---------|-------------|
| `sports` | string | all | Filter by sports |
| `books` | string | all | Target sportsbooks for +EV detection |
...
eventSource.addEventListener('ev_opportunity', (e) => {
  ...
  data.opportunities.forEach(opp => {
    ...
    if (opp.evPercent >= 5) {
      notify(`🔥 ${opp.evPercent}% EV on ${opp.selection} @ ${opp.sportsbook}`);
    }
  });
});
```
